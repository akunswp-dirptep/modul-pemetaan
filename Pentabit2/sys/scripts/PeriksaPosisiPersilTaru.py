...
persil_fokus = "PersilHanyataru"
persil_fokus_path = os.path.join(dataset_path, persil_fokus)
persil_taru_tetap = "PersilTaruTetap"
persil_taru_tetap_path = os.path.join(dataset_path, persil_taru_tetap)
persil_taru_tetap_centroid = "PersilTaruTetapCentroid"
persil_taru_tetap_centroid_path = os.path.join(dataset_path, persil_taru_tetap_centroid)

persil_line = "PersilLinetaru"
...
if arcpy.Exists(persil_taru_tetap):
    arcpy.Delete_management(persil_taru_tetap)
if arcpy.Exists(persil_taru_tetap_path):
    arcpy.Delete_management(persil_taru_tetap_path)
if arcpy.Exists(persil_taru_tetap_centroid):
    arcpy.Delete_management(persil_taru_tetap_centroid)
if arcpy.Exists(persil_taru_tetap_centroid_path):
    arcpy.Delete_management(persil_taru_tetap_centroid_path)

arcpy.MakeFeatureLayer_management(persil_path, persil_fokus, "obj_taru = 'taru'")
arcpy.CopyFeatures_management(persil_fokus, persil_fokus_path)

arcpy.MakeFeatureLayer_management(persil_path, persil_taru_tetap, "obj_taru = 'non taru'")
arcpy.CopyFeatures_management(persil_taru_tetap, persil_taru_tetap_path)
#arcpy.FeatureToPoint_management(persil_taru_tetap_path, persil_taru_tetap_centroid_path, "INSIDE")
...
exp = "get(!SUM_LebarSisi!, !lb_dpn!, !obj_taru!)"
code_block = """
def get(a, b, c):
 if c == 'taru':
  return a
...
arcpy.MakeTableView_management(persil_taru_tetap_path, tbl_persil_tetap)

arcpy.AddJoin_management(persil, "IdBidang", tbl_persil_tetap, "IdBidang")

exp = "get(!Persil_Taru.obj_taru!, !PersilTaruTetap.letak!, !Persil_Taru.letak!)"
code_block = """
def get(a, b, c):
    if a == 'non taru':
        return b
...
exp = "get(!Persil_Taru.obj_taru!, !PersilTaruTetap.s_letak!, !Persil_Taru.s_letak!)"
code_block = """
def get(a, b, c):
    if a == 'non taru':
        return b
...
arcpy.RemoveJoin_management(persil, persil_taru_tetap)
