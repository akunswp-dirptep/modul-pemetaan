...
arcpy.CalculateField_management("temp_jalan", "kls_jln", "'Lokal Setapak'", "PYTHON")
...
if arcpy.Exists("temp_jalan"):
    arcpy.arcpy.Delete_management("temp_jalan")
if arcpy.Exists(os.path.join(dataset_path, "TopologiJaringanJalan")):
    arcpy.arcpy.Delete_management(os.path.join(dataset_path, "TopologiJaringanJalan"))
arcpy.MakeFeatureLayer_management(jaringanjalan_path, "temp_jalan")

arcpy.AddGeometryAttributes_management("temp_jalan", "LENGTH", "METERS", "", "")
#update 15/10/2021
#arcpy.CalculateField_management(jaringanjalan_path, 'P_Jalan', "[LENGTH]", "VB")
arcpy.CalculateField_management(jaringanjalan_path, 'P_Jalan', "!LENGTH!", "PYTHON")
arcpy.DeleteField_management(jaringanjalan_path, 'LENGTH')
...
if arcpy.Exists(os.path.join(ds_kelas_jalan, "JunctionKls7")):
    arcpy.Delete_management(os.path.join(ds_kelas_jalan, "JunctionKls7"))
if arcpy.Exists(os.path.join(ds_kelas_jalan, "JunctionKls6")):
    arcpy.Delete_management(os.path.join(ds_kelas_jalan, "JunctionKls6"))
...
arcpy.MakeFeatureLayer_management(os.path.join(dataset_path, "JunctionFinal"), "temp_jalan", "MAX_s_kls_jln=7")
arcpy.CopyFeatures_management("temp_jalan", os.path.join(ds_kelas_jalan, "JunctionKls7"))
arcpy.MakeFeatureLayer_management(os.path.join(dataset_path, "JunctionFinal"), "temp_jalan2", "MAX_s_kls_jln=6")
arcpy.CopyFeatures_management("temp_jalan2", os.path.join(ds_kelas_jalan, "JunctionKls6"))
...
arcpy.MakeNetworkDatasetLayer_na(nd_path, "JaringanJalan_ND")
