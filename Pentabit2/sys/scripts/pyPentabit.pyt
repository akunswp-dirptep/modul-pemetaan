...
class DeklarasiZonasiUpdate(object):
    ...
    def updateParameters(self, parameters):
        ...
        if parameters[0].altered:
            ...
            appdata = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
            conf_zonasi_path = os.path.join(appdata, "zonasiupdate.dbf")
            conf_file = open(conf_zonasi_path, "r")
            list_config = conf_file.readlines()
            conf_file.close()
            for line in list_config:
    ...
    def execute(self, parameters, messages):
        ...
        appdata = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        messages.AddMessage("== Proses Dimulai ==")
        conf_path = os.path.join(appdata, "zonasiupdate.dbf")
        if not os.path.exists(os.path.dirname(conf_path)):
            try:
                os.makedirs(os.path.dirname(conf_path))
            except OSError as exc:
...
class TransformasiVariabel(object):
    ...
    def updateParameters(self, parameters):
        ...
        if not parameters[0].altered:
            ...
            parameters[0].value = os.path.join(dataset_path, "Persil")
            parameters[1].value = os.path.join(dataset_path, "Sampel_Skor")
        return
    ...
    def execute(self, parameters, messages):
        ...
        for variabel in list_transform:
            ...
            if variabel.split(" ")[1] == 'Inverse':
                ...
                with arcpy.da.UpdateCursor(persil_skor_path, [in_variabel, proses_variabel]) as cursor:
                    for row in cursor:
                        if (row[1] != 0 and not (row[1] is None)):
                            row[0] = 1.0 / row[1]
        ...
        messages.AddMessage("== Tampilkan Layer ==")
        aprx = arcpy.mp.ArcGISProject('CURRENT')
        current_map = aprx.activeMap
        current_map.addDataFromPath(sampel_model_path)
##        current_map.addDataFromPath(persil_model_path)
        messages.AddMessage("== Proses Selesai ==")
        return
...
class EditAnalisisModel_Jalan(object):
    ...
    def execute(self, parameters, messages):
        ...
        if kelasJalan == "Kolektor Sekunder":
            skr = 4
        if kelasJalan == "Lokal Primer":
            skr = 3
        if kelasJalan == "Lokal Sekunder":
            skr = 2
        if kelasJalan == "Lokal Setapak":
            skr = 1
...
class EditKelasJalan(object):
    ...
    def getParameterInfo(self):
        ...
        param0.filter.list = ['Arteri Primer', 'Arteri Sekunder', 'Kolektor Primer', 'Kolektor Sekunder', 'Lokal Primer', 'Lokal Sekunder', 'Lokal Setapak']
        params = [param0]
        return params
...
class EditZonasiSetelahPrediksi(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Tentukan Informasi Zonasi Pada Persil Setelah Prediksi"
        self.description = ""
        self.canRunInBackground = False
    ...
    def execute(self, parameters, messages):
        ...
        sampel_prediksi_edit = "Sampel_Prediksi"
        sampel_model_edit = "Sampel_Model"
        persil_model_edit = "Persil_Model"
        sampel_prediksi_fields = [f.name.lower() for f in arcpy.ListFields(sampel_prediksi_edit)]
        sampel_model_fields = [f.name.lower() for f in arcpy.ListFields(sampel_model_edit)]
        persil_model_fields = [f.name.lower() for f in arcpy.ListFields(persil_model_edit)]

        if 'min_lb_jln' not in sampel_prediksi_fields:
            arcpy.AddField_management(sampel_prediksi_edit, "min_lb_jln", "DOUBLE")
        if 'min_lb_jln' not in sampel_model_fields:
            arcpy.AddField_management(sampel_model_edit, "min_lb_jln", "DOUBLE")
        if 'min_lb_jln' not in persil_model_fields:
            arcpy.AddField_management(persil_model_edit, "min_lb_jln", "DOUBLE")

        if 'zonasi' not in sampel_prediksi_fields or 's_zonasi' not in sampel_prediksi_fields or 'min_lb_jln' not in sampel_prediksi_fields or 'zonasi' not in sampel_model_fields or 's_zonasi' not in sampel_model_fields or 'min_lb_jln' not in sampel_model_fields or 'zonasi' not in persil_model_fields or 's_zonasi' not in persil_model_fields or 'min_lb_jln' not in persil_model_fields:
            arcpy.AddError("")
            arcpy.AddError("")
            arcpy.AddError("Field zonasi, s_zonasi atau min_lb_jln tidak ditemukan.")
            arcpy.AddError("")
        ...
        ada_seleksi = 0
        ada_seleksi = len(arcpy.Describe(persil_model_edit).FIDSet)
        if ada_seleksi > 0:
            rows = arcpy.UpdateCursor(persil_model_edit)
            for row in rows:
                if "ln_zonasi" in persil_model_fields:
                    row.setValue("ln_zonasi", math.log(s_zonasi))
                if "iv_zonasi" in persil_model_fields and s_zonasi != 0:
                    row.setValue("iv_zonasi", 1.0 / s_zonasi)
                row.setValue('zonasi', zonasi)
                row.setValue('s_zonasi', s_zonasi)
                row.setValue('min_lb_jln', min_lb_jln)
                rows.updateRow(row)
        ...
        ada_seleksi = 0
        ada_seleksi = len(arcpy.Describe(sampel_model_edit).FIDSet)
        if ada_seleksi > 0:
            rows = arcpy.UpdateCursor(sampel_model_edit)
            for row in rows:
                if "ln_zonasi" in sampel_model_fields:
                    row.setValue("ln_zonasi", math.log(s_zonasi))
                if "iv_zonasi" in sampel_model_fields and s_zonasi != 0:
                    row.setValue("iv_zonasi", 1.0 / s_zonasi)
                row.setValue('zonasi', zonasi)
                row.setValue('s_zonasi', s_zonasi)
                row.setValue('min_lb_jln', min_lb_jln)
        ...
        ada_seleksi = 0
        ada_seleksi = len(arcpy.Describe(sampel_prediksi_edit).FIDSet)
        if ada_seleksi > 0:
            rows = arcpy.UpdateCursor(sampel_prediksi_edit)
            for row in rows:
                if "ln_zonasi" in sampel_prediksi_fields:
                    row.setValue("ln_zonasi", math.log(s_zonasi))
                if "iv_zonasi" in sampel_prediksi_fields and s_zonasi != 0:
                    row.setValue("iv_zonasi", 1.0 / s_zonasi)
                row.setValue('zonasi', zonasi)
                row.setValue('s_zonasi', s_zonasi)
                row.setValue('min_lb_jln', min_lb_jln)
...
class EditCluster(object):
    ...
    def execute(self, parameters, messages):
        ...
        if ada_seleksi > 0:
            rows = arcpy.UpdateCursor(persil_edit)
            for row in rows:
                if "ln_zonasi" in persil_edit_fields:
                    row.setValue("ln_zonasi", math.log(s_zonasi))
                if "iv_zonasi" in persil_edit_fields and s_zonasi != 0:
                    row.setValue("iv_zonasi", 1.0 / s_zonasi)
