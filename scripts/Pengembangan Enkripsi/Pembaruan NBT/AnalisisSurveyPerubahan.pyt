...
class Hitung_Jarak_Fasilitas(object):
    ...
    def getParameterInfo(self):
        ...
        hanya_null_nol = arcpy.Parameter(
            displayName="Hanya Hitung Jarak Null atau 0",
            name="hanya_null_nol",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input"
        )
        hanya_null_nol.value = False
        ...
        return [
            simpan_temp,
            hanya_update,
            hanya_null_nol,  # Disisipkan di sini
            output_persil
        ]
    ...
    def execute(self, parameters, messages):
        ...
        hanya_null_nol = parameters[2].value # Nilai parameter baru
        ...
        # --- PINDAH KE SINI: Mengambil list fasilitas lebih awal untuk kebutuhan query seleksi ---
        ...
        # 4. Menentukan layer untuk analisis dan melakukan seleksi
        persil_layer = "persil_input"
        self.delete_if_exists(persil_layer)
        arcpy.management.MakeFeatureLayer(update_fc, persil_layer)

        # Membangun Query Seleksi Dinamis
        queries = []
        
        if hanya_update:
            queries.append("(UPPER(status_per) = 'UPDATE')")
            
        if hanya_null_nol:
            existing_fields = [f.name for f in arcpy.ListFields(persil_layer)]
            null_queries = []
            
            for fasilitas in list_fasilitas:
                nama_field = 'JK' + fasilitas
                if nama_field in existing_fields:
                    # Filter field yang Null atau 0
                    null_queries.append(f"({nama_field} IS NULL OR {nama_field} = 0)")
                else:
                    # Jika field belum ada, berarti semua data butuh dihitung
                    null_queries.append("(1=1)")
                    
            if null_queries:
                # Menggunakan OR antar fasilitas (jika salah satu fasilitas belum terhitung, maka akan diproses)
                null_sql = " OR ".join(null_queries)
                queries.append(f"({null_sql})")

        # Eksekusi seleksi jika ada query yang dibuat
        if queries:
            final_query = " AND ".join(queries)
            messages.addMessage(f"== Query Seleksi: {final_query} ==")
        ...
        if jumlah == 0:
            messages.addWarningMessage("== Tidak ada feature yang diproses (Semua mungkin sudah terhitung) ==")
            # Tetap set parameter output agar model builder tidak error
            if simpan_temp:
                parameters[3].value = temp_output_name
            else:
                parameters[3].value = "Persil_Layer"
            return
        ...
        messages.addMessage(f"== Membuat centroid persil untuk {jumlah} data ==")
        arcpy.management.FeatureToPoint(
            persil_layer,
            persilcentroid_path,
            "INSIDE"
        ...
        for fasilitas in list_fasilitas:
            ...
            # 9. Mengambil nilai Total_P_Jalan
        ...
        # 11. Update data panjang rute ke layer persil
        field_to_update = ["IDBIDANG"] + daftar_nama_field
        with arcpy.da.UpdateCursor(persil_layer, field_to_update) as rows:
            for row in rows:
                data = jarak_dict.get(row[0], {})
                for i, field_name in enumerate(daftar_nama_field, start=1):
                    # Hanya diupdate jika ada data jarak yang dikembalikan solver
        ...
        # Indeks bergeser menjadi 3 karena ada tambahan parameter hanya_null_nol
