...
class Update_Luas_Tanah(object):
    ...
    def execute(self,parameters,messages):
        ...
        if not arcpy.Exists(peta_persil_path):

            messages.addErrorMessage(
                "Feature class Peta_Akhir tidak ditemukan"
            )
...
class Analisis_Bentuk_Persil(object):
    ...
    def execute(self,parameters,messages):
        ...
        if not arcpy.Exists(peta_akhir_path):
        ...
        messages.addMessage("== Generate IdBidang ==")
        ...
        messages.addMessage("== Copy persil baru ==")
        ...
        temp_update="temp_persil_update"

        self.delete_if_exists(temp_update)
        ...
        messages.addMessage("== Hitung azimuth ==")

        shape_field=arcpy.Describe(
            persil_split_path
        ).shapeFieldName
        ...
        messages.addMessage("== Hitung bentuk persil ==")
        ...
        self.delete_if_exists("Persil_Update")
        ...
        parameters[0].value="Persil_Update"
...
class Perbaharui_Persil_Layer(object):

    def __init__(self):

        self.label = "Perbaharui Persil Layer"
        self.description = (
            "Update langsung Persil_Layer "
            "berdasarkan Indikator_Perubahan_Persil"
        )
        self.canRunInBackground = False
    ...
    def getParameterInfo(self):
        ...
        output_layer = arcpy.Parameter(
            displayName="Output Persil Layer",
            name="output_persil_layer",
            datatype="GPFeatureLayer",
            parameterType="Derived",
            direction="Output"
        )
    ...
    def execute(
        ...
        persil_layer_path = os.path.join(
            dataset_path,
            "Persil_Layer"
        )
        ...
        messages.addMessage(
            f"Persil lama terhapus: {delete_count}"
        )
        ...
        messages.addMessage(
            (
                f"Persil baru ditambahkan: "
                f"{inserted_count}"
            )
        )

        # =================================================
        # CLEAN DUPLICATE IdBidang
        # =================================================

        if "IdBidang" in editable_fields:

            messages.addMessage(
                "== Validasi duplicate IdBidang =="
            )
            ...
            with arcpy.da.SearchCursor(
                ...
                for row in rows:
                    ...
                    if bidang_id in seen_ids:

                        duplicate_oids.append(
                            oid
                        )
