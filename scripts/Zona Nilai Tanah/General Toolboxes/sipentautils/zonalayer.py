...
def isZonaLayerComply(showPathMessage = True):
    """
    ...
    if len(zona_layers) == 0:
        ...
        try:
            ...
            if hasattr(zona_layer, 'dataSource'):
                ...
                if showPathMessage:
                    arcpy.AddMessage(f'Sukses: Zona Layer ditemukan dan valid. Path: {layer_path}')
                return layer_path
            else:
...
def saveGDB(ws_path: str, gdb_path: str, label: str):
    """
...
def get_config_values():
    ...
    """
    zl_path = isZonaLayerComply(showPathMessage=False)  # Validasi compliance layer zona
