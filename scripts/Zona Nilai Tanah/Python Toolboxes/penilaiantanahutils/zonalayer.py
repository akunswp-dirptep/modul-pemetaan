...
def get_config_values():
    ...
    provinsi = configs['WADMPR']   # Kode Provinsi
    kota = configs['WADMKK']       # Kode Kabupaten/Kota
    ...
    return (dataset_path, tahun, provinsi, kota, coor, gdb_path)

def check_preferred_server():
    """
    Memeriksa apakah konfigurasi 'preferred_server' telah diatur dalam file config.json.
    
    Fungsi ini membaca file config.json dari workspace yang ditentukan dan memeriksa
    apakah ada entri untuk 'preferred_server'. Jika ada, fungsi akan mengembalikan
    nilai tersebut; jika tidak, fungsi akan mengembalikan string kosong.
    
    Returns:
        str: Nilai preferred_server jika ada, atau string kosong jika tidak ada.
    """
    zl_path = is_zona_layer_comply(show_path_message=False)  # Validasi compliance layer zona
    ws_dir = os.path.dirname(os.path.dirname(os.path.dirname(zl_path)))  # Navigasi ke root workspace
    config_path = os.path.join(ws_dir, "config.json")  # Path ke file config
    ...
    # Mengambil nilai preferred_server jika ada
    preferred_server = configs.get('preferred_server', '') if configs else ''
...
def setup_preferred_server_connection(preferred_server: str):
    ...
    # Tambahkan/ubah preferred_server
    configs['preferred_server'] = preferred_server

    # Simpan kembali ke file config.json
    with open(config_path, 'w') as f:
        json.dump(configs, f, indent=4)

    arcpy.AddMessage(f"Preferred server disimpan ke config.json: {preferred_server}")
